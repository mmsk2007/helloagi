...
- `helloagi context-plan --goal "Fix tests/test_cli_contract.py::TestCLIContract::test_context_plan and src/agi_runtime/cli.py" --task-type coding` — show a Context Unrolling plan: selected task-relevant primitives, typed workspace summary with provenance/confidence/observed/generated/verified state, observed file/test artifact references, safe current-working-directory existence verification for referenced repo-relative artifacts, content-safe path/byte/line/kind metadata for present artifacts, static pytest node status for referenced tests without executing repository code or storing test output, and machine-checkable high-risk action-gate readiness/blockers
- `helloagi context-plan --goal "Delete temp build artifacts after confirming scope" --task-type operations` — show risk-identification and scope-verification primitives plus required `action_risk`/`scope` evidence before high-risk operational actions

## Service and health

- `helloagi doctor`
- `helloagi health` — render organism-level health categories (brain, senses, effectors, immune, memory, metabolism, channels, homeostasis, growth), provider configured-vs-usable status with secret-redacted recovery hints, service doctor recovery issues/recommendations, channel extension readiness with missing env/module hints, plus enforced diagnostics-only safe-mode guidance when action-required organs prevent normal operation; `helloagi auto` refuses autonomous steps while that safe mode is active
